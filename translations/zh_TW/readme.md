...
2. <a name="spacing">漂亮的語法</a>
    ...
    // 在外部的小括號內部包住條件式 (condition) 的小括號，沒有空格
    if ( !("foo" in obj) ) {
...
6. <a name="naming">命名</a>
    ...
    宣告一個 `this` 的別名，以 `self` 作變數名。這很有可能出現 bug，應該盡可能避免使用。
...
8. <a name="native">原生 & 宿主物件 (Host Objects)</a>
    ...
    #### “一切都被允許: 擴展內建的方法或物件..等” by Andrew Dupont (JSConf2011, Portland, Oregon)
