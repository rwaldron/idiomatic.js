...
2. <a name="spacing">Красивый Синтаксис</a>
    ...
    // Функция, которая принимает один строковый литерал как параметр, тоже без пробела
    foo("bar");
...
Любой проект, который ссылается на этот документ в качестве руководства по стилю, не примет форматирование кода "первой запятой" (["comma first code formatting"](https://gist.github.com/isaacs/357981)), только если автор специально не укажется этого.
