...
* Alex Navasardyan [@marcotrulla](http://twitter.com/alexnavasardyan), [github](https://github.com/2k00l)
...
### Я не намерен навязывать свои предпочтения в стиле другим людям; собственный стиль написания кода достоен уважения.

> "Чтобы быть хорошим управляющим проекта, нужно понимать, что писать код для себя это Плохая Идея. Если тысячи людей используют ваш код, то пишите свой код максимально понятно, а не на собственное усмотрение." - Idan Gazit.
 ...
 * [Проверка типов (Руководство по стилю от jQuery Core](#type)
...
3. <a name="type">Проверка типов (Руководство по стилю от jQuery Core)</a>
    ...
    // то `importantTask()` никогда бы не была вычислена, даже если значение `foo` было бы "1"
